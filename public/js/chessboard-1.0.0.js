...
;(function () {
  ...
  var numRows = 8
  var numColumns = 8
  var allColumns = 'abcdefghijklmnopqrstuvwxyz'.split('') //one letter for every column
  //allColumns should contain 16 letters (up to p) but will be changed in the future

  var COLUMNS = allColumns.slice(0, numColumns)
  ...
  function validFen (fen) {
    ...
    for (var i = 0; i < numColumns; i++) {
      if (chunks[i].length !== numColumns ||
          chunks[i].search(/[^kqrnbpdlhtKQRNBP1DLHT]/) !== -1) {
        return false
      }
    }
  ...
  function constructor (containerElOrString, config) {
    ...
    function calculateSquareSize () {
      ...
      while (boardWidth % numColumns !== 0 && boardWidth > 0) { //boardWidth -> numRow fix maybe
        boardWidth = boardWidth - 1
      }

      return boardWidth / numColumns
    }

    // create random IDs for elements
    function createElIds () {
      // squares on the board
      for (var i = 0; i < numColumns; i++) { //changed COLUMNS.length to numRows
        for (var j = 1; j <= numRows; j++) { //changed numRows
    ...
    function buildBoardHTML (orientation) {
      ...
      var row = numRows //when 8 -> 9, 9th row is added and everything else shifts down
      if (orientation === 'black') {
        alpha.reverse()
        row = 1
      }
      ...
      for (var i = 0; i < numRows; i++) { //8 -> numRows
        html += '<div class="{row}">'
        for (var j = 0; j < numColumns; j++) {  //8 -> numColumns
          ...
          if (config.showNotation) {
            ...
            if ((orientation === 'white' && row === 1) ||
                (orientation === 'black' && row === numRows)) { //8 -> numRows
        ...
        if (numColumns % 2 == 0) { //if even run this line
          squareColor = (squareColor === 'white') ? 'black' : 'white'
        }
        //squareColor = (squareColor === 'white') ? 'black' : 'white' //old line
    ...
    function buildSparePiecesHTML (color) {
      var pieces = ['wK', 'wQ', 'wR', 'wB', 'wN', 'wP', "wL", "wD", "wH", "wT"]
      if (color === 'black') {
        pieces = ['bK', 'bQ', 'bR', 'bB', 'bN', 'bP', 'bL', "bD", "bH", "bT"]
      }
    ...
    widget.resize = function () {
      ...
      // set board width
      $board.css('width', squareSize * numColumns + 'px')
